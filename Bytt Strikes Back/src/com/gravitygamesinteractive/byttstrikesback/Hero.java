...
public class Hero {
	...
	public void tick(){
		if(health>0){
			if(!isCollidingWithWall(new Point(x+width,y), new Point(x+width+2,y+height-5))){
				x+=3;
				if(x>=Frame.gameSize.width/2 && Level.scrollX<Level.maxx+16-Frame.gameSize.width){
					Level.scrollX+=3;
			...
			if(!isJumping && allowFall && !isCollidingWithFloor(new Point(x,y+height), new Point(x+16,y+height))){
				...
				if(fallspeed<5){
					if(jdecCount>=2){
						...
						y+=fallspeed;
						if(Level.scrollY>0 && Level.scrollY<(Level.maxy+16-(Frame.gameSize.height))){
					...
					y+=fallspeed;
					if(Level.scrollY>0 && Level.scrollY<(Level.maxy-(Frame.gameSize.height))){
						Level.scrollY+=fallspeed;
					}else if(Level.scrollY>=(Level.maxy-(Frame.gameSize.height))){
						Level.scrollY=(Level.maxy-(Frame.gameSize.height));
					}
				}
			}else{
				fallspeed=1;
		...
		if(jumpCount>=jumpHeight){
			if(jumpSpeed==0){
				...
				if(jdecCount>=2){
					...
					y-=jumpSpeed;
					if(Level.scrollY>0 && y-Level.scrollY<(Frame.gameSize.height/2)){
	...
	public void render(Graphics g, Component c){
		if(invincibleCount % 2 == 0){
			//g.drawImage(kylegfx,x-8-Level.scrollX,y-Level.scrollY,null);
			...
			if(animFrame==1 && !allowJump && health>0){
				sproing1.paintIcon(c,g,x-Level.scrollX,y-Level.scrollY);
			}
			if(animFrame==2 && !allowJump && health>0){
				sproing2.paintIcon(c,g,x-Level.scrollX,y-Level.scrollY);
			}
			if(allowJump && health>0){
				sproing3.paintIcon(c,g,x-Level.scrollX,y-Level.scrollY);
			}
		}else{

		}
		if(health<=0){
			sproing4.paintIcon(c,g,x-Level.scrollX,y-Level.scrollY);
